...
public class ThirdRound {
    ...
    public void accept(boolean incremental, ClassWalker walker, TransformProviderFactory manager, TransformInvocation invocation) throws IOException, InterruptedException, TransformException {
        ...
        if (incremental) {
            ...
            graph.getAll().values()
                    .stream()
                    .filter(c -> c.status() == coffeepartner.capt.plugin.api.graph.Status.REMOVED)
            ...
            Future<Map<QualifiedContent, Set<String>>> future = computation.submit(() -> {
                ...
                return Stream.concat(
                        ...
                        transforms.stream()
                                ...
                                .filter(c -> c.status() == coffeepartner.capt.plugin.api.graph.Status.NOT_CHANGED))
                        // ignore to remove
        ...
        boolean isTarget(ApkClassInfo info) {
            switch (request.scope()) {
                case ALL:
                    return true;
                case CHANGED:
                    if (info.status() != coffeepartner.capt.plugin.api.graph.Status.NOT_CHANGED) {
                        return true;
                    }
                default:
